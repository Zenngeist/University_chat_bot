...
if not os.path.exists(VECTOR_STORE_PATH) or not os.path.exists(DOC_STORE_FILE_PATH):
    ...
    st.text_area("Ingestion Log", ingestion_output, height=300)
    st.button("Start Chatbot") 
@st.cache_resource
def load_advanced_rag_chain():
    """
    Loads all components for the Multi-Query ParentDocumentRetriever and the Conversational RAG chain.
    """
    ...
    try:
        ...
        raise FileNotFoundError(f"'{DOC_STORE_FILE_PATH}' not found after ingestion. Please check script permissions.")
...
if len(st.session_state.messages) <= 1:
    ...
    with col1:
        if st.button("🗓️ When is Youthopia fest?"):
            process_query("When is Youthopia fest?")
            st.rerun() 
    with col2:
        if st.button("✍️ What is the Mid-term schedule?"):
            process_query("What is the Mid-term schedule?")
            st.rerun()
    with col3:
        if st.button(f"Today's Time Table ({today_name})"):
            process_query(f"What is my full schedule for {today_name}?")
            st.rerun()
